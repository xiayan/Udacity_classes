...
class FlipPredictor(object):
    def __init__(self,coins):
        ...
        self.probs=[1/n]*n
    def pheads(self):
        return sum([x*y for x, y in zip(self.coins, self.probs)])
    ...
    def update(self,result):
        ...
        p_heads = self.pheads()
        if result == 'H':
            self.probs = [x*y/p_heads for x,y in zip(self.coins, self.probs)]
        else:
            self.probs = [(1-x)*y/(1-p_heads) for x,y in zip(self.coins, self.probs)]
