...
def cfgempty(grammar,symbol,visited):
    if symbol in visited:
        return None
    elif symbol not in [s[0] for s in grammar]:
        return [symbol]
    else:
        visited = visited + [symbol]
        for p in [s[1] for s in grammar if s[0] == symbol]:
            if all(None != cfgempty(grammar, new, visited) for new in p):
                result = []
                for new in p:
                    result += cfgempty(grammar, new, visited)
                return result
    return None
