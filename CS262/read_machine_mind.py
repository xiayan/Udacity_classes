...
def nfsmaccepts(current, edges, accepting, visited):
    ...
    if current in accepting:
        ...
        for (s, l) in edges.keys():
            if (current, s, l) in visited: continue
            visited.append((current, s, l))
            if s == current:
                possible_states = edges[(s, l)]
                for state in possible_states:
                    later = nfsmaccepts(state, edges, accepting, visited)
                    if later != None:
                        return l + later
                return None
