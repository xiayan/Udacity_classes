...
def cfginfinite(grammar):
    nt = [n[0] for n in grammar]
    def traversal(grammar, symbol, visited, target, nt):
        if symbol in visited:
            return False
        visited = visited + [symbol]
        for i in [d for s,d in grammar if s == symbol]:
            if target in i: return True
            else:
                for letter in [l for l in i if l in nt]:
                    if traversal(grammar, letter, visited, target, nt) == True: return True
        return False
    for pair in [d for d in grammar if len(d[1]) > 1]:
        if traversal(grammar, pair[0], [], pair[0], nt) == True: return True
    return False
